...
import com.vincenthzr.locationspoofer.data.model.ImportExportCounts
import com.vincenthzr.locationspoofer.data.model.ImportExportSelection
...
internal suspend fun MainViewModel.saveEnvironmentData(
    ...
    try {
        ...
        if (isConnected && wifiObj.has("connectedWifi")) {
            ...
            val connWifi = com.vincenthzr.locationspoofer.data.db.LocationConnectedWifi(
                locationId = locId,
        ...
        if (nearbyArr != null) {
            for (i in 0 until nearbyArr.length()) {
                ...
                environmentDao.insertWifiDevice(
                    com.vincenthzr.locationspoofer.data.db.WifiDevice(
                        bssid = bssid,
    ...
    try {
        ...
        for (i in 0 until btArr.length()) {
            ...
            environmentDao.insertBluetoothDevice(
                com.vincenthzr.locationspoofer.data.db.BluetoothDevice(
                    address,
                    obj.optString("name", ""),
...
internal fun MainViewModel.locationToJson(
    records: List<com.vincenthzr.locationspoofer.data.db.CompleteLocation>,
    targetLat: Double,
    targetLng: Double
    ...
    val connectedObj = if (explicitWifiBssid == "__NONE__") {
        ...
        org.json.JSONObject().apply {
            ...
            put(
                "channel",
                com.vincenthzr.locationspoofer.utils.MacVendorHelper.frequencyToChannel(
        ...
        org.json.JSONObject().apply {
            ...
            put(
                "channel",
                com.vincenthzr.locationspoofer.utils.MacVendorHelper.frequencyToChannel(cw.frequency)
            )
    ...
    // 2. 插值附近的 Wi-Fi
    val wifiMap = mutableMapOf<String, com.vincenthzr.locationspoofer.data.db.LocationWithWifi>()
    ...
    wifiMap.forEach { (bssid, rw) ->
        ...
        val obj = org.json.JSONObject().apply {
            ...
            put(
                "channel",
                com.vincenthzr.locationspoofer.utils.MacVendorHelper.frequencyToChannel(rw.device.frequency)
            )
    ...
    val cellMap = mutableMapOf<String, com.vincenthzr.locationspoofer.data.db.LocationWithCell>()
    ...
    val btMap =
        mutableMapOf<String, com.vincenthzr.locationspoofer.data.db.LocationWithBluetooth>()
    ...
    viewModelScope.launch(Dispatchers.IO) {
        try {
            ...
            val settings = if (selection.settings) {
                com.vincenthzr.locationspoofer.data.db.ExportedSettings(
            ...
            val apiKeys = if (selection.apiKeys) {
                com.vincenthzr.locationspoofer.data.db.ExportedApiKeys(
...
internal fun MainViewModel.parseImportPackage(
    uri: android.net.Uri,
    onParsed: (com.vincenthzr.locationspoofer.data.db.LocationSpooferDataPackage?) -> Unit
) {
    viewModelScope.launch(Dispatchers.IO) {
        val parsed = try {
...
private fun MainViewModel.parseImportPackageInternal(
    uri: android.net.Uri
): com.vincenthzr.locationspoofer.data.db.LocationSpooferDataPackage? {
    ...
    val pkg = try {
        json.decodeFromString<com.vincenthzr.locationspoofer.data.db.LocationSpooferDataPackage>(jsonStr)
    } catch (e: Exception) {
        null
    }
    ...
    try {
        val legacyLocations =
            json.decodeFromString<List<com.vincenthzr.locationspoofer.data.db.CompleteLocation>>(jsonStr)
        if (legacyLocations.isNotEmpty()) {
            return com.vincenthzr.locationspoofer.data.db.LocationSpooferDataPackage(locations = legacyLocations)
        }
    } catch (e: Exception) {
        try {
            val legacySaved = json.decodeFromString<List<SavedLocation>>(jsonStr)
            if (legacySaved.isNotEmpty()) {
                return com.vincenthzr.locationspoofer.data.db.LocationSpooferDataPackage(savedLocations = legacySaved)
            }
        } catch (e2: Exception) {
            e2.printStackTrace()
        }
...
internal fun MainViewModel.applyImportPackage(
    pkg: com.vincenthzr.locationspoofer.data.db.LocationSpooferDataPackage,
