...
import com.vincenthzr.locationspoofer.viewmodel.MainViewModel.ClusterData
    ...
    viewModelScope.launch {
        locationRepository.getSavedRoutes().collect { entities ->
            val routes = entities.map { entity ->
                ...
                com.vincenthzr.locationspoofer.data.model.SavedRoute(entity.name, points).apply {
...
/**
 * 设置手动选择的厂商适配方案。system_server 里的 [com.vincenthzr.locationspoofer.xposed.hooks.vendor.VendorRegistry]
...
internal fun MainViewModel.requestRestartHookedApps() {
    ...
    val apps = targetPackages.filter { it.isNotBlank() && !exempt.contains(it) }.mapNotNull { pkg ->
        try {
            ...
            com.vincenthzr.locationspoofer.data.model.AppInfoItem(pkg, label, isSystem)
        } catch (_: Exception) {
            com.vincenthzr.locationspoofer.data.model.AppInfoItem(pkg, pkg, false)
        }
    }.sortedBy { it.appName }
...
internal suspend fun MainViewModel.performLocalSearch(): List<AppPoiItem> {
    ...
    return clusters.map { cluster ->
        ...
        com.vincenthzr.locationspoofer.ui.screen.AppPoiItem(
